...
import { ExtractedFactInput, RetrievedFact } from "../../common/types.js";
  ...
  private async reconcileOne(
    ...
    for (let i = 0; i < candidates.length; i++) {
      ...
      if (relation === "contradicts") {
        const inserted = await this.repo.insert(fact, embedding, sourceMessageId);
      ...
      if (relation === "same" || relation === "refines") {
        await this.repo.updateFact(candidates[i].id, fact.object, Math.max(fact.confidence, candidates[i].confidence));
